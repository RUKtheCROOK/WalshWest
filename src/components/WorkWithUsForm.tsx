...
import { workWithUsSchema, type WorkWithUsInput } from "@/lib/validations";
import { submitWorkWithUs } from "@/server/actions";
...
export function WorkWithUsForm() {
  ...
  const onSubmit = async (data: WorkWithUsInput) => {
    setSubmitting(true);
    setMessage(null);

    const result = await submitWorkWithUs(data);
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
      ...
      <div className="space-y-2">
        <Label htmlFor="operation_type" className="text-foreground">Type of Operation (Optional)</Label>
        <Input
          id="operation_type"
          {...register("operation_type")}
          placeholder="e.g., Grain producer, Processor, etc."
          className="transition-shadow focus-visible:shadow-sm"
        />
        {errors.operation_type && (
          <p className="text-sm font-medium text-destructive">{errors.operation_type.message}</p>
        )}
      </div>
